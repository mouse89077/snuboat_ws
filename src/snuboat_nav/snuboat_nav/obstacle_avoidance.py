...
from std_msgs.msg import Bool, Int8, Float64, Float64MultiArray, Int32MultiArray
import numpy as np
...
class Obstacle_Avoidance(Node):
    def __init__(self):
        ...
        self.enu_pos_sub = self.create_subscription(
            Float64, "/enu_pos", self.enu_pos_callback, 1
        )
        self.heading_sub = self.create_subscription(
            Float64, "/nav/heading", self.heading_callback, 1
        )
        self.spd_sub = self.create_subscription(Float64, "/spd", self.spd_callback, 1)
        self.enu_wp_set_sub = self.create_subscription(
            Float64MultiArray, "/enu_wp_set", self.enu_wp_set_callback, 1
        )
    ...
    def enu_wp_set_callback(self, msg):
        ...
        self.wp_reach_check = (
            np.linalg.norm([self.enu_pos[-1, :] - self.enu_wp_set[self.cur_wp_idx, :]])
            < self.goal_tol
        )
    ...
    def cal_des(self):
        if self.wp_reach_check == True and self.wp_time_cnt < self.wp_stay_time:
            ...
            # wp ref heading
            self.ref_heading = np.arctan2(
                self.enu_wp_pos[1] - cur_pos[1], self.enu_wp_pos[0] - cur_pos[0]
            )
            ...
            if len(self.obs_labels) != 0:
                for idx, label in enumerate(self.obs_labels):
                    if self.obs_r[idx] < self.safe_radius:
                        # danger obs
                        danger_heading = self.obs_phi[idx] + self.heading
                        self.danger_heading = np.append(
                            self.danger_heading, danger_heading
                        )
                        continue
                    else:
                        safe_heading = self.obs_phi[idx] + self.heading
